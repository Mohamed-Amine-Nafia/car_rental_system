...
function IntroSection({ isDarkMode, language }) {
  return (
    <section className="flex flex-col">
      <div
        className={`flex flex-col   md:items-center ${language === "fr" ? "md:flex-row" : "md:flex-row-reverse"}`}
      >
        <div
          style={{
            direction: `${language === "fr" ? "ltr" : "rtl"}`,
          }}
          className={`mt-5 md:mt-0 p-5 md:w-1/2 ${language === "fr" ? "text-left" : "text-right"}`}
        >
          <span className="inline-block py-1 px-3 text-xs bg-accent text-secondary rounded-full ">
            {language === "fr"
              ? "Location de voitures à Laayoune"
              : "وكالة كراء السيارات بالعيون"}
          </span>
          <h1
            className={`text-2xl font-semibold leading-relaxed md:text-4xl lg:text-5xl mt-6 ${isDarkMode ? "text-ternary" : "text-secondary"} `}
          >
            {language === "fr"
              ? " Votre prochaine grande évasion"
              : "ابدأ رحلتك القادمة بثقة وراحة كاملة، واختر السيارة التي تناسب أسلوبك"}{" "}
            <span className="italic font-normal text-accent">
              {language === "fr" ? "commence ici" : "من هنا"}
              {"."}
            </span>
          </h1>
          <p
          ...
          >
            {language === "fr"
              ? "   La voiture que vous voulez, au moment exact où vous en avez besoin.Consultez notre inventaire en temps réel et prenez le volant en quelques minutes."
          ...
          <div className="mt-10">
            ...
            >
              {language === "fr" ? "Résérver maintenant" : "احجز سيارتك"}
            </a>
          </div>
        </div>
